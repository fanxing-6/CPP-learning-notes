...
在进程管理结构 `task_struct` [^2]中有几个与信号处理相关的字段
...
<img src="https://mmbiz.qpic.cn/mmbiz_png/ciab8jTiab9J5lRiaQFwf9BYIOWJkibZ3UtwVpWKuWfDN4QKubo1xlgevhUzkyJEFoXTdPiahWywJRibgAXqm2Ty8dcQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1" alt="图片" style="zoom: 67%;" />
...
<img src="https://mmbiz.qpic.cn/mmbiz_png/ciab8jTiab9J5lRiaQFwf9BYIOWJkibZ3Utw3U6ThuMIFGhEbiaw2TXSpYHdtiaa3YpdQ26DUH0VwJnUwnysYgvQNHTg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1" alt="图片" style="zoom:80%;" />
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/ciab8jTiab9J5lRiaQFwf9BYIOWJkibZ3UtwUn6MwKk42Z2F5cu7Pfib0wgAyVgwz0eatXEAgeUXuytrTic300EKGzHg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
这个函数也不难，我们上面介绍过，进程管理结构中有个 `sig` 的字段，它是一个 `struct k_sigaction` 结构的数组，每个元素保存着对应信号的处理程序，所以 `do_sigaction()` 函数就是修改这个信号处理程序。代码 `k = ¤t->sig->action[sig-1]` 就是获取对应信号的处理程序，然后把其设置为新的信号处理程序即可。

[^1]: [Linux 内核如何处理中断 | Linux 中国 (qq.com)](https://mp.weixin.qq.com/s/xPzrziGgtMRWDecVKB4gpg)

[^2]: [Linux并发（task_struct结构体） (qq.com)](https://mp.weixin.qq.com/s/mfoYgPPnAO77uR2u5n91pg)内核中用一个非常大的结构体来表达进程或者线程（统称任务task）
